...
const CONSTRAINTS = [
  {
    ...
    badge: 'STRICT LIMIT',
    badgeColor: 'text-[#050816]/75 bg-slate-900 border-slate-800/30',
  },
  {
    ...
    badge: 'WHITELIST',
    badgeColor: 'text-primary-accent bg-slate-900 border-primary-accent/20',
  },
  {
    title: 'Expiry Rules',
...
export default function ConstraintCards({ isActive }: ConstraintCardsProps) {
  ...
  return (
    <motion.div
      ...
      className="flex flex-col gap-2.5 w-full max-w-[320px] mx-auto py-2"
    >
      {CONSTRAINTS.map((item, index) => (
        <motion.div
          ...
          className="relative bg-slate-900 border border-slate-800/10 px-4 py-2.5 rounded-xl flex items-center justify-between gap-4 backdrop-blur-md shadow-sm h-[68px] w-full overflow-hidden"
        >
          ...
          <div className="flex items-center gap-3.5 min-w-0">
            <div className="p-2 rounded-lg bg-slate-950/40 border border-slate-800/20 shrink-0">
              {item.icon}
            </div>
            <div className="text-left min-w-0">
              ...
              <p className="text-[9.5px] text-slate-400 font-sans font-medium mt-0.5 truncate max-w-[155px]">
                {item.desc}
              </p>
            </div>
          </div>
            ...
            {item.tokens ? (
              <div className="flex gap-1 items-center">
                {item.tokens.map((token) => (
                  <span
                    key={token}
                    className="text-[9px] font-bold bg-slate-950/60 border border-slate-800 text-slate-100 px-1.5 py-0.5 rounded shadow-sm shrink-0"
                  >
                    {token}
                  </span>
                ))}
              ...
              <p className="text-[11px] font-extrabold text-[#050816] tracking-tight font-sans truncate">
                {item.value}
              </p>
            )}
          </div>
          ...
          <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-slate-800/5 overflow-hidden rounded-b-xl">
            <motion.div
              ...
              className={`h-full ${
                index % 2 === 0 ? 'bg-primary-accent' : 'bg-[#050816]/30'
              }`}
            />
          </div>
        </motion.div>
